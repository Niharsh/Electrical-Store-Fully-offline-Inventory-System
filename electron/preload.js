...
contextBridge.exposeInMainWorld("api", {
  ...
  verifyOwnerExists: (username) =>
    ipcRenderer.invoke("verify-owner-exists", username),
  ...
  loginOwner: (username, password) =>
    ipcRenderer.invoke("login-owner", username, password),
  getOwner: () => ipcRenderer.invoke("get-owner"),
  resetPasswordRecovery: (username, recoveryCode, newPassword) =>
    ipcRenderer.invoke(
  ...
  // Wholesalers
  addWholesaler: (wholesalerData) =>
    ipcRenderer.invoke("add-wholesaler", wholesalerData),
  getWholesalers: () => ipcRenderer.invoke("get-wholesalers"),
  updateWholesaler: (id, wholesalerData) =>
    ipcRenderer.invoke("update-wholesaler", id, wholesalerData),
  ...
  addProductType: (typeData) =>
    ipcRenderer.invoke("add-product-type", typeData),
  getProductTypes: () => ipcRenderer.invoke("get-product-types"),
  deleteProductType: (typeId) =>
    ipcRenderer.invoke("delete-product-type", typeId),

  // HSN Codes
  addHSNCode: (hsnData) => ipcRenderer.invoke("add-hsn-code", hsnData),
  getHSNCodes: () => ipcRenderer.invoke("get-hsn-codes"),
  updateHSNCode: (hsnCode, hsnData) =>
    ipcRenderer.invoke("update-hsn-code", hsnCode, hsnData),
  deleteHSNCode: (hsnCode) => ipcRenderer.invoke("delete-hsn-code", hsnCode),
  // HSN GST Report
  getHSNGSTReport: (params) => ipcRenderer.invoke("get-hsn-gst-report", params),
  downloadHSNReport: (payload) =>
    ipcRenderer.invoke("download-hsn-report", payload),
  // Invoices
  createInvoice: (invoiceData) =>
    ipcRenderer.invoke("create-invoice", invoiceData),
  getInvoices: () => ipcRenderer.invoke("get-invoices"),
  getInvoiceById: (invoiceId) =>
    ipcRenderer.invoke("get-invoice-by-id", invoiceId),
  deleteInvoice: (invoiceId) => ipcRenderer.invoke("delete-invoice", invoiceId),
  updateInvoice: (invoiceId, invoiceData) =>
    ipcRenderer.invoke("update-invoice", invoiceId, invoiceData),
  ...
  downloadInvoice: () => ipcRenderer.invoke("download-invoice"),
  downloadInvoiceSilent: () => ipcRenderer.invoke("download-invoice-silent"),
  ...
  saveOrUpdateCustomer: (customerData) =>
    ipcRenderer.invoke("save-or-update-customer", customerData),
  updateCustomer: (customerId, customerData) =>
    ipcRenderer.invoke("update-customer", customerId, customerData),
  ...
  getSalesOverview: (period) =>
    ipcRenderer.invoke("get-sales-overview", period),
  getPurchaseOverview: (period) =>
    ipcRenderer.invoke("get-purchase-overview", period),
  getRecentInvoices: (limit) =>
    ipcRenderer.invoke("get-recent-invoices", limit),

  // Purchase Bills
  createPurchaseBill: (billData) =>
    ipcRenderer.invoke("create-purchase-bill", billData),
  getPurchaseBills: () => ipcRenderer.invoke("get-purchase-bills"),
  updatePurchaseBill: (billId, billData) =>
